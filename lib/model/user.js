...
User.prototype.addToInbox = function(activity, callback) {
    var user = this,
        ...
        // This is terrible.
        isUnseen = function(object, callback) {
            Step(
                function() {
                    ...
                    str.each(
                        function(id, callback) {
                            Step(
                                ...
                                function(err, act) {
                                    if (!err) {
                                        if (act.verb == Activity.POST && act.object && act.object.id == object.id) {
                                            seen = true;
                                        }
                                    }
                                    this(null);
    ...
    Step(
        function() {
            addToMain(user, activity, this);
        },
        function(err) {
            if (err) throw err;
            if (direct) {
                addToDirect(user, activity, this);
            } else {
                this(null);
            }
        },
        function(err) {
            if (err) throw err;
            if (activity.verb != Activity.SHARE) {
                this(null, activity.isMajor());
            } else {
                isUnseen(activity.object, this);
            }
        },
        function(err, isMajor) {
            if (isMajor) {
                addToMajor(user, activity, this.parallel());
                if (direct) {

...
ActivityObject.beforeCreate = function(props, callback) {
    ...
    Step(
        function() {
            if (!_.has(props, "id")) {
                ActivityObject.makeSlug(props, this);
            } else {
        ...
        function(err, slug) {
            ...
            if (slug) {
                ...
                self        = ActivityObject.makeURI("api/" + props._slug);
                props.id    = self;

                props.url   = URLMaker.makeURL(props._slug);
...
ActivityObject.makeSlug = function(props, callback) {

    var now = Date.now(),
        datePart = dateFormat(now, "yyyy/mm/dd", true),
        typePart,
        lastPart,
        nicknamePart,
        slug;
    ...
    if (_.has(props, "objectType") && 
        _.contains(ActivityObject.objectTypes, props.objectType)) {
        typePart = props.objectType;
    } else {
        typePart = "other";
    }

    if (_.has(props, "displayName")) {
        lastPart = ActivityObject.slugify(props.displayName);
    }

    if (_.has(props, "author") &&
        _.isObject(props.author) &&
        _.has(props.author, "preferredUsername")) {
        nicknamePart = props.author.preferredUsername;
    }
    ...
    Step(
        function() {
            ...
            if (!nicknamePart || nicknamePart.length === 0) {
                User = require("./user").User;
                User.fromPerson(props.author.id, cb1);
            } else {
                cb1(null, null);
            }
        ...
        function(err, user, rnd) {
            if (err) {
                callback(err, null);
            } else {
                if (user) {
                    nicknamePart = user.nickname;
                }
                if (rnd) {
                    lastPart = rnd;
                }
                slug = [nicknamePart, typePart, datePart, lastPart].join("/");
                callback(null, slug);
            }
        }
    );
...
ActivityObject.slugify = function(str) {
    return str.trim()
        .toLowerCase()
        .replace(/\s+/g, " ")
        .replace(" ", "_")

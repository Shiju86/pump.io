...
ActivityObject.beforeCreate = function(props, callback) {
    ...
    var type = this.type,
        isLocal = function(url) {
            return urlparse(url).host == URLMaker.makeHost();
        },
        newObject = function(props, callback) {
    ...
    Step(
        function() {
            if (!props.id && (!props.url || isLocal(props.url))) {
                newObject(props, this);
            } else {
                // If we don't have a better ID, use the URL as the ID
